...
namespace DSM.UI.Api.Services
{
    ...
    public class ReportsService : IReportsService
    {
                ...
                var query = this._context.ServerDisks
                    ...
                    .Select(x => new OverallDiskStatusItem
                    {
                        ...
                        FreeDiskSpace = x.DiskFreeSpace.ToString() + " MB (% " +
                                        string.Format("{0:0.##}", 100 * x.DiskFreeSpace / (float)x.DiskCapacity) + ")",
        ...
        {
            IEnumerable<PropertyInfo> stringProperties = typeof(OverallDiskStatusItem).GetProperties()
                .Where(prop => prop.PropertyType == term.GetType());

            var stringPropertyNames = new List<string> { "CompanyName", "ServerName", "Responsible" };

            stringProperties = stringProperties.Where(prop => stringPropertyNames.Contains(prop.Name));

            var query = this._context.ServerDisks.Include(x => x.Server)
                .Select(x => new OverallDiskStatusItem
                {
                    CompanyName = x.Server.Company.Name,
                    ServerId = x.Server.ServerId,
        ...
        {
            IEnumerable<PropertyInfo> stringProperties = typeof(ScheduledJobItem).GetProperties()
                .Where(prop => prop.PropertyType == term.GetType());
        ...
        public IEnumerable<SearchODMItemResult> GetSearchODMItems(object term)
        {
            IEnumerable<PropertyInfo> stringProperties =
                typeof(ODMItem).GetProperties().Where(prop => prop.PropertyType == term.GetType());
        ...
        {
            IEnumerable<PropertyInfo> stringProperties = typeof(KPIMetricsView).GetProperties()
                .Where(prop => prop.PropertyType == term.GetType());
